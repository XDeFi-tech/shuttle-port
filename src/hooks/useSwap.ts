...
function useSwapSimulate({ amount, offerAssetAddress, returnAssetAddress, poolAddress, wallet }: SimulateProps) {
  return useQuery(['swap-simulate', amount, offerAssetAddress, returnAssetAddress, poolAddress], async () => {
    ...
    console.log('swap-simulate', amount, offerAssetAddress, returnAssetAddress, poolAddress)
    ...
    return {
      ...
      beliefPrice: BigNumber(amount).times(getTokenDecimals(offerAssetAddress)).div(response.return_amount).toString(),
      price: BigNumber(amount).times(getTokenDecimals(offerAssetAddress)).div(BigNumber(response.return_amount).div(getTokenDecimals(returnAssetAddress))).toString(),
    };
  }, {
...
export default function useSwap({ amount, offerAssetAddress, returnAssetAddress, poolAddress, slippage = "0.005" }: Props) {
    ...
    if (offerAssetAddress.startsWith("u") || offerAssetAddress === "inj" || offerAssetAddress.startsWith("ibc/")) {
      return [
        new MsgExecuteContract({
          ...
          funds: [{ denom: offerAssetAddress, amount: BigNumber(amount).times(getTokenDecimals(offerAssetAddress)).toString() }],
        })
      ];
    }
